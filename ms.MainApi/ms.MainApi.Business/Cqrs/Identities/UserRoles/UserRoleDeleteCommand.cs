...
public class UserRoleDeleteCommand : IRequest<IMainResponseDto>
{
    public UserRoleCreateDto form { get; }

    public UserRoleDeleteCommand(UserRoleCreateDto _form)
    {
        form = _form;
    }

    public class Handler : IRequestHandler<UserRoleDeleteCommand, IMainResponseDto>
    {
        #region DI
        ...
        private readonly IValidator<UserRoleDeleteCommand> _validator;
        ...
        public Handler(IMapper mapper, IPermissionCheck checkPermission,
            ...
            IValidator<UserRoleDeleteCommand> validator, IMessagesRepository messagesRepository)
        {
            _mapper = mapper;
        ...
        public async Task<IMainResponseDto> Handle(UserRoleDeleteCommand request, CancellationToken cancellationToken)
        {
            #region Validation
            ...
            //Delete Excluded RoleId from DB
            var userRolesForDelete = RoleIds.Where(i => request.form.rolesId.Contains(i));
            foreach (int roleId in userRolesForDelete)
                await _userRoleDal.DeleteAsync(i => i.userId == request.form.userId && i.roleId == roleId);
